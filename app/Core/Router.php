...
class Router
{
  ...
  public static function run()
  {
    ...
    $notFoundControllerClass = 'App\\Controllers\\NotFoundController';
    $action = 'index';

    if (class_exists($notFoundControllerClass)) {
      $controllerInstance = new $notFoundControllerClass();
      if (method_exists($controllerInstance, $action)) {
        http_response_code(404);
        return call_user_func([$controllerInstance, $action]);
      } else {
        http_response_code(500);
        throw new \Exception("Method $action not found in $notFoundControllerClass");
      }
    } else {
      http_response_code(500);
      throw new \Exception("Controller $notFoundControllerClass not found");
    }
  }
}
